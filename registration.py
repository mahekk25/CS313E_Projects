...
class Graph:
    ...
    def compute_depth(self):
        ...
        def helper_depth(i):
            ...
            for neighbor in neighbors:
                neighbor_depth = helper_depth(neighbor)
                max_depth = max(max_depth, neighbor_depth)
            total = 1 + max_depth
            self.vertices[i].depth = total
            computed[i] = total
            return total
    ...
    def get_registration_plan(self):
        ...
        for i, vertex in enumerate(self.vertices):
            if count[i] == 0:
                avail.insert((-vertex.depth, vertex.label, i))

        while not avail.is_empty():
            semester = []
            for _ in range(4):
                if avail.is_empty():
                    break

                _, label, idx = avail.delete()
