...
def prepare_game():
    ...
    with open("secret_words.txt", "r", encoding = "ascii") as f:
        secret_words = [word.strip() for word in f.readlines()]

    # Modify this if statement! This is just starter code.
    secret_word = ""
    if len(sys.argv) != 2:
        raise ValueError("Invalid word choice.")
    arg = sys.argv[1]
    if arg.isalpha() and len(arg) == 5 and arg.islower():
        secret_word = arg
    elif arg.isdigit():
        random.seed(int(arg))
