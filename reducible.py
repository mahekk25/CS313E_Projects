...
def find_word(s, hash_table):
    ...
    """
    # Starts hash and step size
    hashing = hash_word(s, len(hash_table))
    step = step_size(s)
    # Keeps the checker for loop
    initial = hashing
    # works with the fact that space is taken
    while hash_table[hashing] != "":
        if hash_table[hashing] == s:
            return True
        # Keeps searching until it comes back
        hashing = (hashing + step) % len(hash_table)
        if hashing == initial:
            return False
    # Space is None and automatic False
    return False
...
def main():
    ...
    size_m = int(0.2 * word_count + 1)
    while not is_prime(size_m):
        size_m += 1
    ...
    longest_r = get_longest_words(reducible_words)

    # print the reducible words in alphabetical order
    # one word per line
    longest_r.sort()
    for word in longest_r:
        print(word)
